...
def parseArgs():
    ...
    parser.add_argument("--t_a", type=float, default=100.0,
                        dest="t_a", help="Temperature for soft accuracy")
    parser.add_argument("--t_b", type=float, default=0.01,
                        dest="t_b", help="Temperature for soft binning")
...
if __name__ == "__main__":
    ...
    with tqdm.tqdm(total=num_epochs) as pbar_epochs:
        for epoch in range(start_epoch, num_epochs):
            ...
            train_loss, train_ece = train_single_epoch(epoch,
                                                       net,
                                                       train_loader,
                                                       meta_loader_copy,
                                                       optimizer,
            ...
            if (epoch + 1) % args.save_interval == 0:
                save_name = args.save_loc + args.exp_name + \
                    '_' + str(epoch + 1) + '.model'
                torch.save(net.state_dict(), save_name)
